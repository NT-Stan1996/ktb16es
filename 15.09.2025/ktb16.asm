...
; Função para tocar beep de 732Hz uma vez durante a inicialização
initsnd:
    pushf               ; Salvar flags
    push ax
    push bx
    ...
    ; Método alternativo: usar BIOS para beep
    ; Primeiro tentar via hardware direto
    
    ; Desabilitar interrupts temporariamente
    cli
    
    ; Programar PIT Timer 2 (8253/8254)
    mov al, 0xB6        ; Canal 2, acesso LSB+MSB, modo 3
    out 0x43, al
    
    ; Divisor para 732Hz: 1193182 / 732 = 1630
    mov dx, 1630
    mov al, dl          ; LSB
    out 0x42, al
    mov al, dh          ; MSB  
    ...
    ; Conectar timer ao speaker via porta 0x61
    in al, 0x61
    mov bl, al          ; Salvar estado original
    or al, 0x03         ; Ativar gate (bit 0) e speaker (bit 1)
    out 0x61, al
    
    ; Reabilitar interrupts
    sti
    
    ; Delay usando loop simples calibrado
    mov cx, 18          ; ~1 segundo em 18.2Hz ticks
delay_outer:
    push cx
    mov cx, 0x4000      ; Inner delay
delay_inner:
    nop
    loop delay_inner
    pop cx
    ...
    ; Desabilitar interrupts para restaurar
    cli
    
    ; Restaurar estado da porta 0x61
    mov al, bl
    out 0x61, al
